...
func epochIntToBytes(_ epoch: Int) -> Epoch {
...
func dateToEpoch(_ timestamp: Date, _ epochUnitSeconds: Int = DefaultEpochUnitSeconds) -> Epoch {
    let unixTimestamp = Int(timestamp.timeIntervalSince1970)
    let epoch = unixTimestamp / epochUnitSeconds
    return epochIntToBytes(epoch)
}
